...
def escape_markdown(text: str) -> str:
    """Helper function to escape telegram markdown characters V2."""
    escape_chars = r'_*[]()~`>#+-=|{}.!'
...
def send_welcome(message):
    ...
    bot.reply_to(message, f"Welcome to Retrinity cc shop, {escape_markdown(message.from_user.first_name)}!", reply_markup=markup, parse_mode="MarkdownV2")
...
def add_product_start(message):
    ...
    bot.send_message(message.chat.id, "Please upload the \.txt file you want to sell\.", parse_mode="MarkdownV2")
...
def remove_product_start(message):
    ...
    for p in products:
        ...
        button_text = f"❌ {escape_markdown(name)} - {escape_markdown(format_price(price))}"
...
def handle_file_upload(message):
    ...
    if not message.document.file_name.lower().endswith('.txt'):
        bot.send_message(message.chat.id, "Error: Only \.txt files are accepted\.", parse_mode="MarkdownV2")
        return
    try:
        ...
        if error:
            bot.send_message(message.chat.id, f"Error: {escape_markdown(error)}")
            return
        ...
        bot.send_message(message.chat.id, f"📄 File `{escape_markdown(message.document.file_name)}` uploaded\.\n*Now, please enter the price for this item\.*", parse_mode="MarkdownV2")
    except Exception as e:
...
def handle_text_messages(message):
    ...
    if isinstance(user_states.get(user_id), dict) and user_states[user_id].get('state') == 'waiting_price':
        ...
        try:
            ...
            bot.send_message(message.chat.id, "*✅ Product Added Successfully\!*", parse_mode="MarkdownV2")
        ...
        bot.send_message(message.chat.id, "I don't understand that\. Please use the menu buttons\.", parse_mode="MarkdownV2")
    ...
    try:
        if call.data.startswith('product_'):
            ...
            if balance >= price:
                ...
                bot.edit_message_text("You have enough funds to buy this item\.\n\nHow would you like to pay?", call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="MarkdownV2")
            else:
            ...
            if get_user_balance(user_id) >= price:
                ...
                bot.edit_message_text("Your balance is no longer sufficient\.", call.message.chat.id, call.message.message_id, reply_markup=None, parse_mode="MarkdownV2")
...
def show_external_payment_info(call, payment_method, product_id):
    ...
    esc_payment_id = escape_markdown(payment_id[:8])
...
def handle_download_callback(call, access_token):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, "Failed to send the file\. Please contact support\.", parse_mode="MarkdownV2")
...
def handle_download(message, access_token):
    ...
    try:
        ...
        debug_print(f"Download error: {str(e)}")
        bot.send_message(message.chat.id, "Failed to send the file\. Please contact support\.", parse_mode="MarkdownV2")

# (The rest of the handlers should also be reviewed for markdown safety if they send formatted text)
